...
4. `tools/summarize_with_gemini.py` — three one-line bullets ranked by urgency. The body is capped at `BODY_CHAR_LIMIT` (4,000 chars) before the Gemini call.
...
- The exact run time and cron expression live in [CLAUDE.md](../CLAUDE.md) (one home for the schedule). Verify the droplet clock with `ssh root@167.71.232.223 date` before installing.
...
Edit [config/employees.py](../config/employees.py), commit, push, `git pull` on the droplet. The next daily cycle picks up the change. No restart needed.
